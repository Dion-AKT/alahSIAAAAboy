...
def main_app():
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # Perbaikan: hindari duplikat nama sheet
        used_sheets = set()
        for akun, df_akun in ledger_adj.items():
            safe_sheet = re.sub(r'[\[\]:\\/*?]', '_', akun)[:25]
            sheet_base = f"Buku - {safe_sheet}"
            sheet_name = sheet_base
            counter = 1
            while sheet_name.lower() in used_sheets:
                sheet_name = f"{sheet_base}_{counter}"
                counter += 1
            used_sheets.add(sheet_name.lower())
            df_akun.to_excel(writer, sheet_name=sheet_name[:31], index=False)
